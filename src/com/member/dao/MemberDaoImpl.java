...
public class MemberDaoImpl implements MemberDao, SqlInter {
	...
	public int getInsert(MemberVo memberVo) {
		...
		int res = 0;
		try {
			System.out.println("insert를 해보자");
			res = session.insert("memberMapper.memberInsert", memberVo);
			System.out.println("insert가 끝났다");
	...
	public int getUpdate(MemberVo memberVo) {
		System.out.println("다오  update ... " + memberVo.getId());
		System.out.println(memberVo);
		SqlSession session = factory.openSession();
		int all = 0;
		try {
	...
	@Override
	public MemberVo getFind(MemberVo vo) {
		SqlSession session = factory.openSession();
		
		MemberVo all = null;
		try {
			all = session.selectOne("memberMapper.memberFind", vo);
		} finally {
			session.close();
		}
